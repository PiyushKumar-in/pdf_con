...
class _CaptchaState extends State<Captcha> {
  double value = 0;
  late double randomHeight, randomWidth;
  late String url;
  late Image img;
  bool _done = false;
  ...
  @override
  void initState() {
    randomHeight = ran.nextDouble() * (widget.size - widget.boxSize);
    randomWidth =
        ran.nextDouble() * (widget.size - widget.boxSize * 2) +
        widget.boxSize +
        10;
    url =
        "https://raw.githubusercontent.com/ServiceStack/images/refs/heads/master/hero/${images[ran.nextInt(images.length - 1)]}";
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Slider(
          min: 0,
          max: (widget.size - widget.boxSize),
          value: value,
          allowedInteraction: SliderInteraction.slideThumb,
          onChangeEnd: (value) {
            if ((value - randomWidth).abs() < 5) {
              setState(() {
                _done = true;
              });
              ...
              setState(() {
                ...
                img = Image.network(url, fit: BoxFit.cover);
                randomHeight =
                    ran.nextDouble() * (widget.size - widget.boxSize);
                randomWidth =
                    ran.nextDouble() * (widget.size - widget.boxSize * 2) +
                    widget.boxSize +
                    10;
